/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { EmployeeEnrollmentWhereInput } from "./EmployeeEnrollmentWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { EmployeeEnrollmentOrderByInput } from "./EmployeeEnrollmentOrderByInput";

@ArgsType()
class EmployeeEnrollmentFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => EmployeeEnrollmentWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => EmployeeEnrollmentWhereInput, { nullable: true })
  @Type(() => EmployeeEnrollmentWhereInput)
  where?: EmployeeEnrollmentWhereInput;

  @ApiProperty({
    required: false,
    type: [EmployeeEnrollmentOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [EmployeeEnrollmentOrderByInput], { nullable: true })
  @Type(() => EmployeeEnrollmentOrderByInput)
  orderBy?: Array<EmployeeEnrollmentOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { EmployeeEnrollmentFindManyArgs as EmployeeEnrollmentFindManyArgs };
